...
> ## ⚠️ 本文已合并，不再单独维护（2026-09-19）
>
> 本文件内容已**完整合并**进 `THREE_STRATEGY_PLAN.md` §6「S4：Shizuku 特权通道」，
> 并在合并时做了修订：
>
> - Chain-3 正式登记为 **S4 策略**（原为「待决策 SQ2」）
> - `NetworkFilterMode` 枚举补充 `APP_FIREWALL` / `HYBRID`
> - 让位逻辑增加「有 Shizuku 时降级到 S4」的分支
> - 风险清单与实施优先级并入主体方案的阶段 F 与 §12
>
> **后续所有规划请以 `THREE_STRATEGY_PLAN.md` 为准。**
> 本文件仅作历史记录保留，其中的内容可能已过时。

> 目录：`reference/`（技术方案，非最终代码）
> 前置阅读：`THREE_STRATEGY_PLAN.md`（四策略主体方案）
> 编制日期：2026-09-19
> 状态：**已归档（内容已合并至主体方案 §6）**
